		...
		<table style="width: 100%" class="data">
			...
			<tbody>
				...
				<c:forEach var="package" items="${packages}" varStatus="packageStat">
					<url:modify var="downloadUrl" value="${currentUrl}">
						<url:clearParams />
						<url:addParam name="packageName" value="${package.name}" />
			...
			<c:forEach var="package" items="${packages}" varStatus="packageStat">
				...
				<tr class="row-heading">
					<td colspan="6">
						...
						<c:if test="${fn:length(package.fileSets) > maximumVersionCount}">
							<a class="action" href="${downloadUrl}">show all releases</a>
						</c:if>
					</td>
				</tr>
				<c:forEach var="fileSet" items="${package.fileSets}" varStatus="fileSetStat">
					<c:if test="${fileSetStat.index < maximumVersionCount}">
						<c:choose>
