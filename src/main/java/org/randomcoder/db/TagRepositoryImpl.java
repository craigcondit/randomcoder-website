...
public class TagRepositoryImpl implements TagRepositoryCustom {
  private static final String QUERY_ALL_TAG_STATISTICS =
      "select t, size(t.articles) from Tag t group by t order by t.displayName";
  private static final String NATIVE_QUERY_MOST_ARTICLES =
      "select max(c) from (select article_id, count(1) c from article_tag_link group by article_id) x";
  ...
  @Override public List<TagStatistics> findAllTagStatistics() {
    Query query = entityManager.createQuery(QUERY_ALL_TAG_STATISTICS);
    return buildTagStatistics(query);
  }

  @Override public Page<TagStatistics> findAllTagStatistics(Pageable pageable) {
  ...
  @Override public int maxArticleCount() {
    Number result =
        (Number) entityManager.createNativeQuery(NATIVE_QUERY_MOST_ARTICLES)
            .getSingleResult();
