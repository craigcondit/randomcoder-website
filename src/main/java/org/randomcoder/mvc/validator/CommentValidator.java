 ...
 */
@Component("commentValidator") public class CommentValidator
    implements Validator {
  private static final String ERROR_COMMENT_NULL = "error.comment.null";
  private static final String ERROR_COMMENT_USER_NAME_FORBIDDEN =
      "error.comment.username.forbidden";
  private static final String ERROR_COMMENT_USER_NAME_REQUIRED =
      "error.comment.username.required";
  private static final String ERROR_COMMENT_EMAIL_ADDRESS_FORBIDDEN =
      "error.comment.emailaddress.forbidden";
  private static final String ERROR_COMMENT_EMAIL_ADDRESS_REQUIRED =
      "error.comment.emailaddress.required";
  private static final String ERROR_COMMENT_EMAIL_ADDRESS_INVALID =
      "error.comment.emailaddress.invalid";
  private static final String ERROR_COMMENT_WEBSITE_FORBIDDEN =
      "error.comment.website.forbidden";
  private static final String ERROR_COMMENT_WEBSITE_INVALID =
      "error.comment.website.invalid";
  private static final String ERROR_COMMENT_TITLE_REQUIRED =
      "error.comment.title.required";
  private static final String ERROR_COMMENT_CONTENT_REQUIRED =
      "error.comment.content.required";
  private static final String ERROR_COMMENT_CONTENT_INVALID =
      "error.comment.content.invalid";

  private static final Logger logger =
      LoggerFactory.getLogger(CommentValidator.class);
  ...
  @Override public void validate(Object obj, Errors errors) {
    ...
    if (command.isAnonymous()) {
      ...
      if (username == null) {
        errors
            .rejectValue("anonymousUserName", ERROR_COMMENT_USER_NAME_REQUIRED,
                "user name required");
      }

      String emailAddress = command.getAnonymousEmailAddress();
      if (emailAddress == null) {
        errors.rejectValue("anonymousEmailAddress",
            ERROR_COMMENT_EMAIL_ADDRESS_REQUIRED, "email address required");
      } else if (!DataValidationUtils.isValidEmailAddress(emailAddress)) {
        errors.rejectValue("anonymousEmailAddress",
            ERROR_COMMENT_EMAIL_ADDRESS_INVALID, "email address invalid");
      }

      String website = command.getAnonymousWebsite();
      if (website != null && !DataValidationUtils.isValidUrl(website)) {
        errors.rejectValue("anonymousWebsite", ERROR_COMMENT_WEBSITE_INVALID,
            "web site invalid");
      }
    } else {
      ...
      if (command.getAnonymousEmailAddress() != null) {
        errors.reject(ERROR_COMMENT_EMAIL_ADDRESS_FORBIDDEN,
            "email address forbidden");
      }
    ...
    if (title == null || title.trim().length() == 0) {
      errors
          .rejectValue("title", ERROR_COMMENT_TITLE_REQUIRED, "title required");
    }
    ...
    if (content == null || content.trim().length() == 0) {
      errors.rejectValue("content", ERROR_COMMENT_CONTENT_REQUIRED,
          "content required");
