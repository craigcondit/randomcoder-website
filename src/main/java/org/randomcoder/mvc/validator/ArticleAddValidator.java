 ...
 */
@Component("articleAddValidator") public class ArticleAddValidator
    implements Validator {
  private static final String ERROR_ARTICLE_CONTENT_INVALID =
      "error.article.content.invalid";
  private static final String ERROR_ARTICLE_CONTENT_REQUIRED =
      "error.article.content.required";
  private static final String ERROR_ARTICLE_SUMMARY_INVALID =
      "error.article.summary.invalid";
  private static final String ERROR_ARTICLE_SUMMARY_TOO_LONG =
      "error.article.summary.toolong";
  private static final String ERROR_ARTICLE_CONTENT_TYPE_REQUIRED =
      "error.article.contentType.required";
  private static final String ERROR_ARTICLE_TITLE_REQUIRED =
      "error.article.title.required";
  private static final String ERROR_ARTICLE_NULL = "error.article.null";
  private static final String ERROR_ARTICLE_PERMALINK_INVALID =
      "error.article.permalink.invalid";
  private static final int DEFAULT_MAX_SUMMARY_LENGTH = 1000;

  private static final Logger logger =
      LoggerFactory.getLogger(ArticleAddValidator.class);

  /**
   * Message resource for permalink exists message.
   */
  protected static final String ERROR_ARTICLE_PERMALINK_EXISTS =
      "error.article.permalink.exists";
   ...
   */
  @Inject @Named("contentFilter") public void setContentFilter(
   ...
   *
   * @param articleBusiness article business object
   */
   ...
   */
  @Value("${article.max.summary.length}") public void setMaximumSummaryLength(
   ...
   */
  @Override public boolean supports(Class<?> givenClass) {
  ...
  @Override public void validate(Object obj, Errors errors) {
    ...
    if (errors.getFieldErrorCount("permalink") == 0) {
      ...
      if (permalink != null) {
        ...
        if (prev != null) {
          errors.rejectValue("permalink", ERROR_ARTICLE_PERMALINK_EXISTS,
              "permalink exists");
        }
      }
    }
  ...
  protected final boolean validateCommon(ArticleAddCommand command,
    ...
    if (title == null || title.trim().length() == 0) {
      errors
          .rejectValue("title", ERROR_ARTICLE_TITLE_REQUIRED, "title required");
    }

    String permalink = command.getPermalink();
    if (permalink != null && !permalink.matches("[a-z0-9\\-]+")) {
      errors.rejectValue("permalink", ERROR_ARTICLE_PERMALINK_INVALID,
          "permalink invalid");
    }

    ContentType contentType = command.getContentType();
    if (contentType == null) {
      errors.rejectValue("contentType", ERROR_ARTICLE_CONTENT_TYPE_REQUIRED,
          "content type required");
    }

    String content = command.getContent();
    if (content == null || content.trim().length() == 0) {
      errors.rejectValue("content", ERROR_ARTICLE_CONTENT_REQUIRED,
          "content required");
    } else if (contentType != null) {
      // validate the content
      validateContent(errors, content, contentType.getMimeType(), "content",
          ERROR_ARTICLE_CONTENT_INVALID, "content invalid");
    }
    ...
    if (summary != null) {
      ...
      if (summary.length() > maximumSummaryLength) {
        errors.rejectValue("summary", ERROR_ARTICLE_SUMMARY_TOO_LONG,
            new Object[] { Integer.valueOf(maximumSummaryLength) },
            "summary too long");
      } else {
        validateContent(errors, summary, contentType.getMimeType(), "summary",
            ERROR_ARTICLE_SUMMARY_INVALID, "summary invalid");
      }
    }
  ...
  private void validateContent(Errors errors, String content, String mimeType,
      String fieldName, String errorMessage, String fallbackMessage) {
    ...
    try {
      ...
      errors.rejectValue(fieldName, errorMessage,
          new Object[] { Integer.valueOf(line), Integer.valueOf(col),
