 ...
 */
@Component("changePasswordValidator") public class ChangePasswordValidator
    implements Validator {
  private static final int DEFAULT_MINIMUM_PASSWORD_LENGTH = 6;

  private static final String ERROR_OLD_PASSWORD_REQUIRED =
      "error.changepassword.oldpassword.required";
  private static final String ERROR_OLD_PASSWORD_NO_MATCH =
      "error.changepassword.oldpassword.nomatch";

  private static final String ERROR_PASSWORD_REQUIRED =
      "error.changepassword.password.required";
  private static final String ERROR_PASSWORD_TOO_SHORT =
      "error.changepassword.password.tooshort";
  private static final String ERROR_PASSWORD_NO_MATCH =
      "error.changepassword.password.nomatch";
  ...
  @Override public void validate(Object obj, Errors errors) {
    ...
    if (user.getPassword() == null) {
      ...
      if (oldPassword != null && oldPassword.length() > 0)
        errors.rejectValue("oldPassword", ERROR_OLD_PASSWORD_NO_MATCH,
            "Old password is wrong.");
    } else {
      // validate old password
      if (oldPassword == null || oldPassword.trim().length() == 0) {
        errors.rejectValue("oldPassword", ERROR_OLD_PASSWORD_REQUIRED,
