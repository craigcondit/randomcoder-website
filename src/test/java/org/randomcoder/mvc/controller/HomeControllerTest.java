...
public class HomeControllerTest {
  ...
  @Test public void testListArticlesBetweenDates() {
  ...
  @Test public void testListArticlesBeforeDate() {
    Date endDate = new Date();
    List<Article> articles = new ArrayList<>();
    ...
    expect(ab.listArticlesBeforeDate(eq(endDate), capture(pageCap)))
        .andReturn(page);
    control.replay();

    assertSame(page,
        c.listArticlesBeforeDate(null, endDate, PageRequest.of(0, 50)));
    control.verify();
  ...
  @Test public void testHome() {
    ...
    expect(tb.getTagCloud())
        .andStubReturn(Collections.<TagCloudEntry>emptyList());
    ...
    assertEquals("home",
        c.home(new ArticleListCommand(), m, PageRequest.of(0, 10), r));
    control.verify();
  }
}