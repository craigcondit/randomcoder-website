...
public class AccountCreateValidatorTest {
  ...
  @Test public void testSupports() {
    assertTrue("Validator doesn't support command class",
        validator.supports(AccountCreateCommand.class));
  }
  ...
  @Test public void testValidate() {
    ...
    assertEquals("Wrong error count for password", 0,
        errors.getFieldErrorCount("password"));
    assertEquals("Wrong error count for password2", 0,
        errors.getFieldErrorCount("password2"));
    ...
    errors = new BindException(command, "command");
    validator.validate(command, errors);
    assertEquals("Wrong error count for website", 1,
        errors.getFieldErrorCount("website"));

    // valid website
    command.setWebsite("http://www.exmaple.com/");
    errors = new BindException(command, "command");
    validator.validate(command, errors);
    assertEquals("Wrong error count for website", 0,
        errors.getFieldErrorCount("website"));
